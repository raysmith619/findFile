...
class Properties(object):
    ...
    def processPair(self, key, value):
        ...
        ### 2.x: if self._keymap.has_key(key):
    ...
    def load(self, stream):
        ...
        ### 2.x:    if type(stream) is not file:
        if not isinstance(stream, io.IOBase):
            raise TypeError('Argument should be a file object!')
            ### 2.x: raise TypeError,'Argument should be a file object!'

        # Check for the opened mode
        if stream.mode != 'r':

            raise ValueError('Stream should be opened in read-only mode!')
            ### 2.x: raise ValueError,'Stream should be opened in read-only mode!'
    ...
    def setProperty(self, key, value):
        ...
        if type(key) is str and type(value) is str:
            ...
            ### 2.x: raise TypeError,'both key and value should be strings!'
    ...
    def store(self, out, header=""):
        ...
        if out.mode[0] != 'w':
            ...
            ### 2.x: raise ValueError,'Steam should be opened in write mode!'
...
if __name__=="__main__":
    ...
    """    
    prop_file_name = 'test4.properties'
    print("Test file: %s" % (prop_file_name))
    if not os.path.exists(prop_file_name):
        test_string = "\n".join(["b.c=b_dot_c", "c.d=c_dot_d", 'e.f="e=f"', "g = h"])
        print("Creating test file %s\ncontaining:\n%s" % (prop_file_name, test_string))
        f = open(prop_file_name, 'w')
        f.write(test_string)
        f.close()                    
        
    
    
    p.load(open(prop_file_name))
    p.list()
    print(p)
    ### 2.x: print p
    print(p.items())
    ...
    print(p['name3'])    
    p['new key'] = 'new value'
    prop_new_name = "new_" + prop_file_name
    print("Write out updated properties into %s" % prop_new_name)
    p.store(open(prop_new_name,'w'))
