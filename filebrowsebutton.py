...
# 19Apr2018  - Ray Smith - Python3 changes
#
...
class FileBrowseButtonWithHistory( FileBrowseButton ):
    ...
    with following additions:
        ...
        New entries are added to history
        """
    ...
    if wx.Platform == "__WXMSW__":
        def SetValue (self, value, callBack=1):
            ...
            if callBack==1:
                ...
                lev_value = LocalEvent(value)
                if lev_value is not None:
                    self.changeCallback(lev_value)


    def OnBrowse (self, event = None):
        """
        Going to browse for file...
        place at top of history, removing identical entry, if present
        """
        super().OnBrowse(event)
        file = self.GetValue()
        hctrl = self.GetHistory()
        if file in hctrl:
            hctrl.remove(file)
        hctrl.append(file)
        self.SetHistory(value=hctrl)
        print("Added")
...
class DirBrowseButtonWithHistory(FileBrowseButtonWithHistory):
    ...
    def __init__(self, parent, id = -1,
                 ...
                 dialogClass = wx.DirDialog,
                 newDirectory = False,
                 name = 'dirBrowseButtonWithHistory'):
        FileBrowseButtonWithHistory.__init__(self, parent, id, pos, size, style,
                                  labelText, buttonText, toolTip,
                                  dialogTitle, startDirectory,
                                  initialValue = initialValue,
                                  changeCallback = changeCallback,
    ...
    def OnBrowse(self, ev = None):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            if dialog_path is not None:
                self.SetValue(dialog_path, callBack=0)      # Hack to avoid non-callable error
        dialog.Destroy()
        """
        Add to history, same as  FileBrowse... but with required style
        can's see a way of using super()
        """
        file = self.GetValue()
        hctrl = self.GetHistory()
        if file in hctrl:
...
if __name__ == "__main__":
    ...
    class DemoFrame( wx.Frame ):
        def __init__(self, parent):
            ...
            middlecontrol = DirBrowseButtonWithHistory(
                panel,
                labelText = "Dirs With History",
